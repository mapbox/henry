...
util.inherits(Henry, events.EventEmitter);
...
Henry.prototype.refresh = function(fn) {
    ...
    request({
        ...
        if (err) {
            ...
            return self.emit('error', error(res));
        }
        url += roles.split('\n')[0];
        request({url: url, json: true}, function(err, res, credentials) {
            if (err) {
                ...
                return self.emit('error', new Error('Could not set key/secret/token from valid response'));
            }
            credentials = {
            ...
            self.emit('refresh', credentials);
            self.removeListener('error', fn);
            fn(null, credentials);
        });
    });
};
