...
var Henry = module.exports = exports = function Henry(client) {
    // Don't manage a client with apparently valid credentials
    if (client.key.length === 20 && client.secret.length === 40) return;
    // Fetch credentials on instantiation
...
exports.updateClient = function(client, fn) {
...
function log(err, code, body) {
    console.warn('%s Henry: Could not refresh credential cache. ' +
      'Error: %s Code: %s Body: %s', ts(), err.toString(), code, body);
}
